...
def process_chunk(chunk, chunk_id):
    ...
    for id in range(len(chunk)):
        ...
        cur.execute("select charttime, valuenum from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(51) +
        ...
        cur.execute("select charttime, valuenum from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(8368) +
        ...
        cur.execute("select charttime, value from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and itemid = " + str(3348) +
                    " order by charttime")
        vitals.append(cur.fetchall())
        cur.execute("select charttime, value from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(115) +
                    " or itemid = 223951) order by charttime")
        vitals.append(cur.fetchall())
        cur.execute("select charttime, value from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(8377) +
                    " or itemid = 224308) order by charttime")
        vitals.append(cur.fetchall())

        # Urine output
        cur.execute("select charttime, VALUE from outputevents where hadm_id = "
                    + str(chunk[id][0]) + " and ( itemid = 40405 or itemid = " +
        ...
        cur.execute("select charttime, valuenum from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(2981) +
        ...
        # Glucose
        cur.execute("select charttime, valuenum from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(807) +
        ...
        # pH
        cur.execute("select charttime, valuenum from chartevents where hadm_id = "
                    + str(chunk[id][0]) + " and (itemid = " + str(780) +
        ...
        vitals.append(cur.fetchall())
        data.append(vitals)
    pickle.dump(data, open(f'vitals_records_chunk_{chunk_id}.p', 'wb'))

def split_list(data, num_chunks):
    """Splits the list into specified number of chunks."""
    chunk_size = len(data) // num_chunks
    return [data[i:i + chunk_size] for i in range(0, len(data), chunk_size)]

# Split list_adm_id into 15 parts
chunks = split_list(list_adm_id[:1000], 20)

# Use ThreadPoolExecutor to run the function on each chunk in parallel
